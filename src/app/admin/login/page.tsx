...
import { ADMIN_SESSION_COOKIE, hasAdminAccessKey, isAdminAuthenticated, validateAdminAccessKey } from "@/lib/admin-auth";
...
export default async function AdminLoginPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const hasKey = hasAdminAccessKey();
      ...
      <div className="grid w-full gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section className="surface-card rounded-[36px] p-8 sm:p-10">
          ...
          <p className="editorial-body mt-5 max-w-2xl text-base text-slate-600">
            Halaman admin disembunyikan dari pengguna publik. Masuk dengan access key untuk mengelola katalog, review cafe, dan modul komersial berikutnya.
          </p>

          {!hasKey ? (
            <div className="mt-8 rounded-[28px] border border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-900">
              ADMIN_ACCESS_KEY belum dipasang di environment. Set dulu agar login admin bisa digunakan dengan aman.
            </div>
          ) : null}

          {params.error === "invalid" ? (
            <div className="mt-8 rounded-[28px] border border-rose-200 bg-rose-50 px-5 py-4 text-sm text-rose-700">
              Access key tidak cocok. Coba lagi.
            </div>
          ) : null}
        </section>
        ...
        <section className="surface-card rounded-[36px] p-8">
          ...
          <form action={loginAction} className="mt-5 space-y-5">
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Access key</span>
              <input
                name="accessKey"
                type="password"
                placeholder="Masukkan access key admin"
